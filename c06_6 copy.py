...
# List to store messages before writing batch_data
batch_data = []
hdfs_path = ""
def process_message(message):
    try:
        ...
        f_b_month = str(int(str(f_batchid)[4:6]))  # fixed conversion to int
        f_b_day = str(int(str(f_batchid)[6:8]))  # fixed conversion to int
        ...
        decoded_message = offset1.decode('utf-8')  # Or use 'latin-1' if applicable
        ...
        try:
            ...
            logging.error(f"Error decoding message: {decoded_message}")
            return
        ...
        if len(batch_data) == 0:
            logging.warning("⚠️ Warning: batch_data is empty, forcing write with dummy row")
            dummy_data = generate_dummy_row()
